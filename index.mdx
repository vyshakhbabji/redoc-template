---
title: Demo API Portal
---

import {
  LandingLayout,
  Button,
  Box,
  FlexSection,
  Flex,
  Jumbotron,
  ThinTile,
  WideTile,
  Emphasis,
  H1,
  H2,
  SectionHeader,
  NavBar,
} from '@redocly/developer-portal/ui';

import icon1 from './images/icon1.png';
import launchFastIcon from './images/launch-fast.svg';
import icon3 from './images/icon3.png';
import bookManagementIcon from './images/book-management.svg';
import foundOrPrivate from './images/found-or-private.svg';

export default LandingLayout;

<Jumbotron>
  <NavBar location={props.location} standalone={false} />
  <H1>Shippo Developer Portal</H1>
  <H2>We make Shipping easier</H2>
  <Flex p={20} justifyContent="center">
    {/* <Button size="xlarge" to="https://app.redoc.ly">
      Redocly app
    </Button> */}
     <Button variant="outlined" size="xlarge" to="developer-portal/index.md">
      Get started
    </Button>
    {/* <Button variant="outlined" size="xlarge" to="https://redoc.ly/docs">
      Docs
    </Button> */}
  </Flex>
</Jumbotron>
<Box my={25}>
  <SectionHeader>
    Create your <Emphasis> developer portal </Emphasis> today!
  </SectionHeader>
  <FlexSection justifyContent="space-around" flexWrap="wrap">
    <ThinTile to="developer-portal/index.md" icon={icon1} header="API Reference">
        Lorem ipsum dolor sit amet, pri putent oportere quaerendum in, ea mea justo invenire
        aliquando. Usu modo suavitate ea. Et sed labitur epicurei adipiscing. Nibh zril labitur
        an usu.Lorem ipsum dolor sit amet, pri putent oportere quaerendum in, ea mea justo
        invenire aliquando. Usu modo suavitate ea. Et sed labitur epicurei adipiscing. Nibh zril
        labitur an usu.Lorem ipsum dolor sit amet, pri putent oportere quaerendum in, ea mea
        justo invenire aliquando. Usu modo suavitate ea. Et sed labitur epicurei adipiscing.
    </ThinTile>
    <ThinTile to="developer-portal/index.md" icon={launchFastIcon} header="Developer Guide">
        Lorem ipsum dolor sit amet, pri putent oportere quaerendum in, ea mea justo invenire
        aliquando. Usu modo suavitate ea. Et sed labitur epicurei adipiscing. Nibh zril labitur
        an usu.
    </ThinTile>
    <ThinTile to="contact.mdx" icon={icon3} header="QuickStart Guide">
        Lorem ipsum dolor sit amet, pri putent oportere quaerendum in, ea mea justo invenire
        aliquando. Usu modo suavitate ea. Et sed labitur epicurei adipiscing. Nibh zril labitur
        an usu.
    </ThinTile>
  </FlexSection>
  {/* <Jumbotron parallaxEffect bgImage={icon1}>
    <H1>Tutorials</H1>
    <H2>Get setup faster...</H2>
  </Jumbotron> */}

[//]: # (<SectionHeader>)

[//]: # (  <Emphasis>Need help? </Emphasis>)

[//]: # (  Try our training exercises.)

[//]: # (</SectionHeader>)

[//]: # (<FlexSection justifyContent="space-around" flexWrap="wrap">)

[//]: # (  <WideTile to="developer-portal/index.md" header="API Reference">)

[//]: # (      Lorem ipsum dolor sit amet, pri putent oportere quaerendum in, ea mea justo invenire)

[//]: # (      aliquando. Usu modo suavitate ea. Et sed labitur epicurei adipiscing. Nibh zril labitur)

[//]: # (      an usu.)

[//]: # (  </WideTile>)

[//]: # (  <WideTile to="developer-portal/index.md" header="Developer Guide">)

[//]: # (      Lorem ipsum dolor sit amet, pri putent oportere quaerendum in, ea mea justo invenire)

[//]: # (      aliquando. Usu modo suavitate ea. Et sed labitur epicurei adipiscing. Nibh zril labitur)

[//]: # (      an usu.)

[//]: # (  </WideTile>)

[//]: # (  <WideTile to="developer-portal/index.md" header="QuickStart Guide">)

[//]: # (      Lorem ipsum dolor sit amet, pri putent oportere quaerendum in, ea mea justo invenire)

[//]: # (      aliquando. Usu modo suavitate ea. Et sed labitur epicurei adipiscing. Nibh zril labitur)

[//]: # (      an usu.)

[//]: # (  </WideTile>)

[//]: # (  <WideTile to="developer-portal/index.md" header="Go">)

[//]: # (      Lorem ipsum dolor sit amet, pri putent oportere quaerendum in, ea mea justo invenire)

[//]: # (      aliquando. Usu modo suavitate ea. Et sed labitur epicurei adipiscing. Nibh zril labitur)

[//]: # (      an usu.)

[//]: # (  </WideTile>)

[//]: # (</FlexSection>)
</Box>

